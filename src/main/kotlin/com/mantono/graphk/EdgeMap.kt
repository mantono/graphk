...
operator fun <T, V> EdgeMap<T, V>.get(key: T): MutableSet<Edge<T, V>> where T: Any, V: Edge<T, V>
{
	return this[key] ?: HashSet<Edge<T, V>>().also { this[key] = it }
}

fun <T, V> edgeMapOf(edges: Map<T, Set<Edge<T, V>>>): EdgeMap<T, V> where T: Any, V: Edge<T, V>
{
	return edges
